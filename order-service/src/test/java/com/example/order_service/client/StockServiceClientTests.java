  ...
  public void verifyProductStock_WhenStockIsAvailable_ShouldPassSuccessfully() {
    ...
    assertDoesNotThrow(() -> stockServiceClient.verifyProductStocks(items));
      ...
      verify(restTemplate, times(1))
        .postForObject(eq(mockUrl), any(), eq(Void.class));
  }

  @Test
  public void verifyProductStock_WhenStockServiceReturnErrorWithStockNotAvailableException_ShouldThrowStockNotAvailableException() throws JsonProcessingException {
    ...
    HttpClientErrorException.BadRequest badRequestException = (HttpClientErrorException.BadRequest)
    ...
    when(restTemplate.postForObject(eq("/check"), anyList(), eq(Void.class)))
        ...
        assertThrows(
            ...
            () -> stockServiceClient.verifyProductStocks(items));

  }

  // --- KỊCH BẢN 3: ĐÁP ỨNG TỪ CAMEL TRẢ VỀ LỖI NGHIỆP VỤ (HTTP 400 Bad Request) ---
  @Test
  public void verifyProductStock_WhenStockServiceReturnErrorWithInvalidStockException_ShouldThrowInvalidStockException() throws JsonProcessingException {
